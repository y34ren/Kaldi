...
if [ $stage -le 2 ]; then
  ...
  utils/subset_data_dir.sh --shortest data/valid_train 500 data/train_10kshort || exit 1;
  echo "stage 2.3 --------------------------------------------------------------------"
  #utils/subset_data_dir.sh data/valid_train 20000 data/train_20k || exit 1;
  utils/subset_data_dir.sh data/valid_train 1000 data/train_20k || exit 1;
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/train_deltas.sh --boost-silence 1.25 --cmd "$train_cmd" \
    100 500 data/train_20k data/lang exp/mono_ali_train_20k exp/tri1
...
if [ $stage -le 5 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" \
    --splice-opts "--left-context=3 --right-context=3" 125 750 \
...
if [ $stage -le 6 ]; then
  echo "stage 6.0 --------------------------------------------------------------------"
  steps/train_sat.sh --cmd "$train_cmd" 125 750 \
...
if [ $stage -le 7 ]; then
  ...
  steps/train_sat.sh  --cmd "$train_cmd" 100 500 \
    data/valid_train data/lang \
...
if [ $stage -le 8 ]; then
  echo "stage 8.0 --------------------------------------------------------------------"
  local/chain/run_tdnn.sh --stage 0 --use-gpu false
fi
